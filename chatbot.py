...
INTERRUPTION_THRESHOLD_FRAMES = 20 # Number of speech frames (30ms each) to trigger an interruption (20 * 30ms = 0.6s)
...
class VADListener:
    def __init__(self, text_queue, whisper_model, bot_is_speaking_event, audio_player):
    ...
    # MODIFIED: A new, simpler, and more robust listening loop to prevent race conditions.
    def listen_loop(self):
        ...
        frames = []
        silence_frames = 0
        silence_timeout_frames = VAD_SILENCE_TIMEOUT // VAD_FRAME_MS
                ...
                try:
                    ...
                    if not triggered:
                        ...
                        if len([f for f, speech in ring_buffer if speech]) > 0.8 * ring_buffer.maxlen:
                            ...
                            triggered = True
                            frames.extend([f[0] for f in ring_buffer])
                            ring_buffer.clear()
                    else:
                        frames.append(data)

                        # --- UNIFIED INTERRUPTION & END-OF-SPEECH LOGIC ---
                        # 1. Check for interruption while recording
                        if self.bot_is_speaking_event.is_set() and len(frames) > INTERRUPTION_THRESHOLD_FRAMES:
                            if self.audio_player.is_playing.is_set(): # Check flag to prevent multiple interrupts
                                print("\nInterrupting Linie...")
                                self.audio_player.interrupt()

                        # 2. Check for end of speech
                        if not is_speech:
                            silence_frames += 1
                            if silence_frames > silence_timeout_frames:
                                print("End of speech detected.")
                                audio_to_transcribe = b''.join(frames)
                                threading.Thread(target=self._transcribe_audio, args=(audio_to_transcribe,)).start()
...
class Chatbot:
    def __init__(self):
        ...
        self.listener = VADListener(self.text_queue, self.whisper, self.bot_is_speaking, self.audio_player)
    ...
    def speak(self, text):
        ...
        try:
            with SuppressTTSOutput():
                tts_stream = self.tts.tts(
                    text=text,
                    speaker_wav=VOICE_TO_CLONE_PATH,
                    language="en",
                    speed=TTS_SPEED
                )

            for audio_chunk in tts_stream:
                audio_chunk_np = np.array(audio_chunk)
                audio_chunk_int16 = (audio_chunk_np * 32767).astype(np.int16)
                self.audio_player.add_to_queue(audio_chunk_int16.tobytes())
        ...
        try:
            while True:
                try:
                    ...
                    if any(x in user_input.lower() for x in ["goodbye", "exit", "shut down"]):
                        self.speak_blocking("Fine. Leaving."); break
